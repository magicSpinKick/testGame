...
LPDIRECT3DINDEXBUFFER9 g_pIPlane = NULL;
LPDIRECT3DVERTEXBUFFER9 g_pVPlane = NULL;
int g_vtxSizePlane = 0;
int g_faseSizePlane = 0;

LPDIRECT3DINDEXBUFFER9 g_pIBox = NULL;
...
void update(LPDIRECT3DVERTEXBUFFER9& g_PVtx, int size, Matrix44& mt);
...
void Render(int timeDelta)
{
		...
		)))
	{
		...
		rx.SetRotationX(0.08f);
//		update(g_pVB, g_VtxSize, rx);
		...
		g_pDevice->SetIndices(g_pIBox);
		g_pDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, g_vtxSizeBox, 0, g_faseSizeBox);

		g_pDevice->SetStreamSource(0, g_pVPlane, 0, sizeof(Vertex));
		g_pDevice->SetIndices(g_pIPlane);
		g_pDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, g_vtxSizePlane, 0, g_faseSizePlane);
...
{
	Vertex* vertices;
	if (FAILED(g_PVtx->Lock(0, sizeof(Vertex), (void**)&vertices, 0))) return;
...
bool InitVertexBuffer()
{
	ReadModelFile("sphere2.dat", g_pVB, g_VtxSize, g_pIB, g_FaceSize);
	ReadModelFile("box.dat", g_pVBox, g_vtxSizeBox, g_pIBox, g_faseSizeBox);
	ReadModelFile("plane.dat", g_pVPlane, g_vtxSizePlane, g_pIPlane, g_faseSizePlane);
